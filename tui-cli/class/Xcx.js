...
class Xcx {
    ...
    next() {
        ...
        if (!requests.length) {
            return;
        }global.dd_class_Xcx_172=requests
        ...
        util_1.log.newline();global.dd_class_Xnc_181=xcxEntry
        this.transfromFromEntry(xcxEntry);global.dd_class_Xnc_182 = util_1
    ...
    watchChange(file) {
        ...
        if (isProjectConfig) {
            ...
            try{
                let page_list =util_1.customConfig.extwxss
                page_list.forEach(function(addr){
                    util_1.xcxNext.watchChangeFile(addr);
                    this.next();
                })
            }catch(err){console.log("wxss error")}
        }
        else {global.dd_class_Xnc_291=file
